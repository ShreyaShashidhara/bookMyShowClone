...
const TheatreList = () => {
  ...
  const getAuthToken = () => localStorage.getItem('token') || jwtToken;
  ...
  const handleAddTheatre = (e) => {
    ...
    fetch("http://localhost:5010/api/theatre", {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) {
          throw new Error(data.message || 'Unable to add theatre');
        }
        return data;
      })
      .then((data) => {
        setTheatres((currentTheatres) => [...currentTheatres, data]);
        setNewTheatre({
          name: '',
          location: '',
          phone: '',
  ...
  useEffect(() => {
    fetch("http://localhost:5010/api/theatre", {
      ...
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.message || 'Unable to load theatres');
        }
        return data;
      })
